...
void _help(char *argv[])
{
    printf("使用:/DataSharingPlatform/tools/bin/syncinc logfilename xmlbuffer\n\n");

    printf("把T_ZHOBTMIND1@db128表中全部的记录增量同步到T_ZHOBTMIND2中。\n");
    printf("事例:/DataSharingPlatform/tools/bin/procctl 10 /DataSharingPlatform/tools/bin/syncinc /log/idc/syncinc_ZHOBTMIND2.log "
    ...
    printf("       /DataSharingPlatform/tools/bin/procctl 10 /DataSharingPlatform/tools/bin/syncinc /log/idc/syncinc_ZHOBTMIND3.log "
           "\"<localconnstr>idc/idcpwd@snorcl11g_128</localconnstr><remoteconnstr>idc/idcpwd@snorcl11g_128</remoteconnstr>"
           "<charset>Simplified Chinese_China.AL32UTF8</charset>"
           "<remotetname>T_ZHOBTMIND1</remotetname><lnktname>T_ZHOBTMIND1@db128</lnktname>"
    ...
    printf("where         同步数据的条件，填充在select starg.remotekeycol from remotetname where starg.remotekeycol>:1之后，注意，不要加where关键字，但是，需要加and关键字，本参数可以为空。\n");
...
bool _syncinc(bool &bcontinue)
{
    ...
    // insert into T_ZHOBTMIND3(obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid) select obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid from T_ZHOBTMIND1@db128 where rowid in (:1,:2,:3);
    sqlstatement stmtins(&connloc); // 向本地表中插入数据的SQL语句。
    stmtins.prepare("insert into %s(%s) select %s from %s where rowid in (%s)", starg.localtname, starg.localcols, starg.remotecols, starg.lnktname, bindstr.c_str());
    for (int ii = 0; ii < starg.maxcount; ii++)
    {
