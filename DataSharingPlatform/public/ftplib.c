...
// 适用于需要它的愚蠢操作系统的FtpInit（Windows NT）
...
#endif
		memcpy((char *)&sin.sin_addr, /*phe->h_addr*/ phe->h_addr_list[0], phe->h_length);
	}
...
// 发送SYST命令
// 用远程系统类型填充用户缓冲区。如果需要来自响应的更多信息，用户可以从FtpLastResponse（）返回的响应缓冲区中解析它。
// 如果命令成功，则返回1，否则返回0
