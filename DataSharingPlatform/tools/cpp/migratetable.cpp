...
void _help()
{
    printf("使用:/DataSharingPlatform/tools/bin/migratetable logfilename xmlbuffer\n\n");

    printf("事例:/DataSharingPlatform/tools/bin/procctl 3600 /DataSharingPlatform/tools/bin/migratetable /log/idc/migratetable_ZHOBTMIND1.log "
    ...
    printf("starttime   程序运行的时间区间，例如02,13表示：如果程序运行时，踏中02时和13时则运行，其它时间不运行,"
           "如果starttime为空，本参数将失效，只要本程序启动就会执行数据迁移，为了减少对数据库的压力，数据迁移一般在业务最闲的时候时进行。\n");
