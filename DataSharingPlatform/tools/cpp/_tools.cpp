...
bool ctcols::allcols(connection &conn, char *tablename)
{
    ...
    stmt.prepare("select lower(column_name),lower(data_type),data_length from USER_TAB_COLUMNS where table_name=upper(:1) order by column_id", tablename);
    ...
    if (stmt.rpc() > 0)
    {
        deleterchr(m_allcols, ','); // obtid,ddatetime,....,keyid
    }
...
bool ctcols::pkcols(connection &conn, char *tablename)
{
    ...
    stmt.prepare("select lower(column_name),position from USER_CONS_COLUMNS where table_name=upper(:1)\
                ...
                where table_name=upper(:2) and constraint_type='P' and generated='USER NAME') order by position");
    stmt.bindin(1, tablename, 30);
    ...
    if (stmt.rpc() > 0)
    {
        deleterchr(m_pkcols, ','); // 删除m_pkcols最后一个多余的逗号。
    }
