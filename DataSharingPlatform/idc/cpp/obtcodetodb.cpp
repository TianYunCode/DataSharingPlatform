...
int main(int argc, char *argv[])
{
    ...
    {
        printf("\n");
        printf("使用:./obtcodetodb inifile connstr charset logfile\n");

        printf("事例:/DataSharingPlatform/tools/bin/procctl 120 /DataSharingPlatform/idc/bin/obtcodetodb /DataSharingPlatform/idc/ini/stcode.ini "
    ...
    stmtins.prepare("insert into T_ZHOBTCODE(obtid,cityname,provname,lat,lon,height,keyid) values(:1,:2,:3,:4*100,:5*100,:6*10,SEQ_ZHOBTCODE.nextval)");
    ...
    stmtupt.prepare("update T_ZHOBTCODE set cityname=:1,provname=:2,lat=:3*100,lon=:4*100,height=:5*10,upttime=sysdate where obtid=:6");
...
bool loadstcode(const string &inifile)
{
    ...
    // for (auto &aa : stcodelist)
    // {
    //     logfile.wri te("provname=%s,obtid=%s,cityname=%s,lat=%s,lon=%s,height=%s\n", aa.provname, aa.obtid, aa.cityname, aa.lat, aa.lon, aa.height);
    // }
