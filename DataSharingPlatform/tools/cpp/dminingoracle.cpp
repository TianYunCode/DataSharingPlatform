...
bool _dminingoracle()
{
    ...
    string strxmlfilename; // 输出的xml文件名，例如：ZHOBTCODE_20240919162835_togxpt_1.xml
    ...
    while (true)
    {
        ...
        if (ofile.isopen() == false)
        {
            ...
            sformat(strxmlfilename, "%s/%s_%s_%s_%d.xml", starg.outpath, starg.bfilename, ltime1("yyyymmddhh24miss").c_str(), starg.efilename, iseq++);
...
void _help()
{
    printf("使用:/DataSharingPlatform/tools/bin/dminingoracle logfilename xmlbuffer\n\n");

    printf("事例:/DataSharingPlatform/tools/bin/procctl 3600 /DataSharingPlatform/tools/bin/dminingoracle /log/idc/dminingoracle_ZHOBTCODE.log "
    ...
    printf("       /DataSharingPlatform/tools/bin/procctl   30 /DataSharingPlatform/tools/bin/dminingoracle /log/idc/dminingoracle_ZHOBTMIND.log "
           "\"<connstr>idc/idcpwd@snorcl11g_128</connstr><charset>Simplified Chinese_China.AL32UTF8</charset>"
    ...
    printf("maxcount    输出xml文件的最大记录数，默认是0，表示无限制，如果本参数取值为0，注意适当加大timeout的取值，防止程序超时。\n");
    printf("starttime   程序运行的时间区间，例如02,13表示：如果程序启动时，踏中02时和13时则运行，其它时间不运行。"
           "如果starttime为空，表示不启用，只要本程序启动，就会执行数据抽取任务，为了减少数据源数据库压力，抽取数据的时候，如果对时效性没有要求，一般在数据源数据库空闲的时候时进行。\n");
    printf("incfield    递增字段名，它必须是fieldstr中的字段名，并且只能是整型，一般为自增字段。如果incfield为空，表示不采用增量抽取的方案。");
