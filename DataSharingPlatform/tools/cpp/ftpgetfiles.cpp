...
struct st_arg
{
    ...
    int mode;                // 传输模式，1-被动模式，2-主动模式，默认采用被动模式。
    ...
    bool checkmtime;         // 是否需要检查服务端文件的时间，true-需要，false-不需要，默认为false。
...
{
    printf("\n");
    printf("使用:/DataSharingPlatform/tools/bin/ftpgetfiles logfilename xmlbuffer\n\n");
    printf("事例:/DataSharingPlatform/tools/bin/procctl 30 /DataSharingPlatform/tools/bin/ftpgetfiles /log/idc/ftpgetfiles_test.log "
    ...
    printf("<mode>1</mode> 传输模式，1-被动模式，2-主动模式，默认采用被动模式。\n");
    ...
    printf("<okfilename>/idcdata/ftplist/ftpgetfiles_test.xml</okfilename> 已下载成功文件名清单，此参数只有当ptype=1时才有效。\n");
    printf("<checkmtime>true</checkmtime> 是否需要检查服务端文件的时间，true-需要，false-不需要，此参数只有当ptype=1时才有效，默认为false。\n");
...
bool _xmltoarg(const char *strxmlbuffer)
{
    ...
    getxmlbuffer(strxmlbuffer, "mode", starg.mode); // 传输模式，1-被动模式，2-主动模式，默认采用被动模式。
    if (starg.mode != 2)
    {
        starg.mode = 1;
    }
    ...
    if (starg.ptype == 1)
    {
        ...
        // 是否需要检查服务端文件的时间，true-需要，false-不需要，此参数只有当ptype=1时才有效，默认为false。
