...
namespace wty
{
    ...
    OCI_SUCCESS_WITH_INFO      1  // maps to SQL_SUCCESS_WITH_INFO   执行成功，但有诊断消息返回，可能是警告信息，但是，在测试的时候，我还从未见识到OCI_SUCCESS_WITH_INFO是怎么回事
    ...
    OCI_STILL_EXECUTING    -3123  // OCI would block error 服务环境建立在非阻塞模式，OCI函数调用正在执行中
    OCI_CONTINUE          -24200  // Continue with the body of the OCI function 说明回调函数需要OCI库恢复其正常的处理操作
    ...
    */
    ...
    {
        return OCIBindByPos(m_handle.smthp, &m_handle.bindhp, m_handle.errhp, (ub4)position, &value, sizeof(value), SQLT_INT, NULL, NULL, NULL, 0, NULL, OCI_DEFAULT);
    }
    ...
    {
        return OCIBindByPos(m_handle.smthp, &m_handle.bindhp, m_handle.errhp, (ub4)position, &value, sizeof(value), SQLT_FLT, NULL, NULL, NULL, 0, NULL, OCI_DEFAULT);
    }
    ...
    {
        value.resize(len);
        return OCIBindByPos(m_handle.smthp, &m_handle.bindhp, m_handle.errhp, (ub4)position, &value[0], len + 1, SQLT_STR, NULL, NULL, NULL, 0, NULL, OCI_DEFAULT);
    }

    int sqlstatement::bindin1(const unsigned int position, string &value)
    {
        return OCIBindByPos(m_handle.smthp, &m_handle.bindhp, m_handle.errhp, (ub4)position, &value[0], value.size() + 1, SQLT_STR, NULL, NULL, NULL, 0, NULL, OCI_DEFAULT);
    }

    int sqlstatement::bindout(const unsigned int position, int &value)
    {
        return OCIDefineByPos(m_handle.smthp, &m_handle.defhp, m_handle.errhp, position, &value, sizeof(value), SQLT_INT, NULL, NULL, NULL, OCI_DEFAULT);
    }
    ...
    {
        return OCIDefineByPos(m_handle.smthp, &m_handle.defhp, m_handle.errhp, position, &value, sizeof(value), SQLT_FLT, NULL, NULL, NULL, OCI_DEFAULT);
    }
    ...
    {
        value.resize(len);
        return OCIDefineByPos(m_handle.smthp, &m_handle.defhp, m_handle.errhp, position, &value[0], len + 1, SQLT_STR, NULL, NULL, NULL, OCI_DEFAULT);
    }
    ...
    int sqlstatement::execute()
    {
        ...
        if (m_sqltype == true)
        {
            OCIAttrGet((CONST dvoid *)m_handle.smthp, OCI_HTYPE_STMT, (dvoid *)&m_cda.rpc, (ub4 *)0, OCI_ATTR_ROW_COUNT, m_handle.errhp);
...
#define LOBMAXBUFLEN 10240
    ...
    int sqlstatement::filetolob(FILE *fp)
    {
        ...
        {
            // 文件中正好只有一块，不需要流写入
            if ((retval = OCILobWrite(m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset, (dvoid *)bufp, (ub4)nbytes, OCI_ONE_PIECE, (dvoid *)0,
                                      (sb4(*)(dvoid *, dvoid *, ub4 *, ub1 *))0, (ub2)0, (ub1)SQLCS_IMPLICIT)) != OCI_SUCCESS)
            {
                err_report();
                return m_cda.rc;
            }
        ...
        {
            // more than one piece
            if (OCILobWrite(m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset, (dvoid *)bufp, (ub4)LOBMAXBUFLEN, OCI_FIRST_PIECE, (dvoid *)0,
                            (sb4(*)(dvoid *, dvoid *, ub4 *, ub1 *))0, (ub2)0, (ub1)SQLCS_IMPLICIT) != OCI_NEED_DATA)
            {
                err_report();
                return m_cda.rc;
            }
            ...
            do
            {
                ...
                retval = OCILobWrite(m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset, (dvoid *)bufp, (ub4)nbytes, piece, (dvoid *)0,
    ...
    int sqlstatement::lobtofile(FILE *fp)
    {
        ...
        retval = OCILobRead(m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset, (dvoid *)bufp, (loblen < LOBMAXBUFLEN ? loblen : LOBMAXBUFLEN), (dvoid *)0,
                            (sb4(*)(dvoid *, const dvoid *, ub4, ub1))0, (ub2)0, (ub1)SQLCS_IMPLICIT);

        switch (retval)
        {
        case OCI_SUCCESS: // 只有一块
            fwrite((void *)bufp, (size_t)amtp, 1, fp);
            break;

        case OCI_NEED_DATA: // 有2件或更多件
            ...
            while (1)
            {
                ...
                retval = OCILobRead(m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset, (dvoid *)bufp, (ub4)LOBMAXBUFLEN, (dvoid *)0,
                                    (sb4(*)(dvoid *, const dvoid *, ub4, ub1))0, (ub2)0, (ub1)SQLCS_IMPLICIT);
