...
// <obtid>58015</obtid><ddatetime>20240908113000</ddatetime><t>141</t><p>10153</p><u>44</u><wd>67</wd><wf>106</wf><r>9</r><vis>102076</vis><keyid>6127135</keyid>
...
{
    printf("使用:/DataSharingPlatform/tools/bin/xmltodb logfilename xmlbuffer\n\n");

    printf("事例:/DataSharingPlatform/tools/bin/procctl 10 /DataSharingPlatform/tools/bin/xmltodb /log/idc/xmltodb_vip.log "
           "\"<connstr>idc/idcpwd@snorcl11g_128</connstr><charset>Simplified Chinese_China.AL32UTF8</charset>"
           "<inifilename>/DataSharingPlatform/idc/ini/xmltodb.xml</inifilename>"
...
bool _xmltodb()
{
    ...
    while (true) // 每循环一次，执行一次入库任务。
    {
        ...
        while (true)
        {
            ...
            if (ret == 0)
            {
                logfile << "ok(" << stxmltotable.tname << ",总数=" << totalcount << ",插入=" << inscount << ",更新=" << uptcount << "，耗时=" << timer.elapsed() << ").\n";
...
{
    // 拼接插入表的SQL语句。
    // insert into T_ZHOBTMIND1(obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid) values(:1,to_date(:2,'yyyymmddhh24miss'),:3,:4,:5,:6,:7,:8,:9,SEQ_ZHOBTMIND1.nextval)
    ...
    // 拼接更新表的SQL语句。
    // update T_ZHOBTMIND1 set t=:1,p=:2,u=:3,wd=:4,wf=:5,r=:6,vis=:7 where obtid=:8 and ddatetime=to_date(:9,'yyyymmddhh24miss')
    // a）拼接update语句开始的部分。
