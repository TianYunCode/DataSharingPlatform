...
namespace wty
{
    ...
    public:
        ...
        // 登录数据库。
        // connstr：数据库的登录参数，格式：username/password@tnsname，username-用户名，password-登录密码，tnsname-数据库的服务名，在$ORACLE_HOME/network/admin/tnsnames.ora文件中配置。
        // charset：客户端的字符集，必须与数据库保持一致，否则会出现中文乱码的情况。
        // autocommitopt：是否启用自动提交事务，false-不启用，true-启用，默认是不启用。
    ...
    public:
        ...
        // len：如果输出变量的数据类型是字符串，用len指定它的最大长度，建议采用表对应的字段长度，如果len的值太小，内容将会被截断。
        // 返回值：0-成功，其它失败，程序中一般不必关心返回值。
