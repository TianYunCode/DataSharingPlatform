...
int main(int argc, char *argv[])
{
    ...
    {
        printf("\n");
        printf("使用 :./inetd logfile inifile\n\n");
        printf("事例:./inetd /tmp/inetd.log /etc/inetd.conf\n\n");
        printf("        /DataSharingPlatform/tools/bin/procctl 5 /DataSharingPlatform/tools/bin/inetd /tmp/inetd.log /etc/inetd.conf\n\n");
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // 把定时器加入epoll。 int tfd = timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK | TFD_CLOEXEC); // 创建timerfd。
    ...
    while (true) // 进入事件循环。
    {
        ...
        for (int ii = 0; ii < infds; ii++)
        {
            ...
            ////////////////////////////////////////////////////////
            // 如果定时器的时间已到，有两件事要做：1）更新进程的心跳；2）清理空闲的客户端socket，路由器也会这么做。 if (evs[ii].data.fd == tfd)
            // {
            ...
            if (evs[ii].events & EPOLLOUT)
            {
                ...
                // 以下代码模拟不能一次发完全部数据的场景。
                // int ilen;
                // if (clientbuffer[evs[ii].data.fd].length() > 10)
                // {
                //     ilen = 10;
                // }
                // else
                // {
                //     ilen = clientbuffer[evs[ii].data.fd].length();
                // }
